...
class DashboardNotifier extends StateNotifier<AsyncValue<DashboardStats>> {
  ...
  DashboardNotifier(this._db) : super(const AsyncValue.loading()) {
    loadStats();
  }
  ...
  Future<void> loadStats() async {
    try {
      ...
      // Match both date formats: dd-MM-yyyy and yyyy-MM-dd
      final todayDDMM =
          '${today.day.toString().padLeft(2, '0')}-${today.month.toString().padLeft(2, '0')}-${today.year}';
      final todayYYYY =
          '${today.year}-${today.month.toString().padLeft(2, '0')}-${today.day.toString().padLeft(2, '0')}';

      final s1 = await db.rawQuery(
          "SELECT COALESCE(SUM(net_amount),0) as t FROM sales WHERE date LIKE ? OR date LIKE ?",
          ['$todayDDMM%', '$todayYYYY%']);
      final s2 = await db.rawQuery(
          "SELECT COALESCE(SUM(amount),0) as t FROM collections WHERE date LIKE ? OR date LIKE ?",
          ['$todayDDMM%', '$todayYYYY%']);
      final s3 = await db.rawQuery(
          "SELECT COALESCE(SUM(current_balance),0) as t FROM customers WHERE current_balance > 0");
      final s4 = await db.rawQuery(
          "SELECT COALESCE(SUM(current_stock * sale_price),0) as t FROM products");
      final lowStock = await db.rawQuery(
          "SELECT * FROM products WHERE current_stock <= min_stock_level LIMIT 5");
      final recent = await db.rawQuery('''
        SELECT s.id, s.date, s.net_amount, s.payment_type, c.shop_name, c.phone
        FROM sales s 
