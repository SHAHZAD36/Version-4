...
final purchasesProvider = StateNotifierProvider<PurchaseNotifier, List<PurchaseModel>>(
    (ref) => PurchaseNotifier(ref.watch(purchaseRepoProvider), ref));

class PurchaseNotifier extends StateNotifier<List<PurchaseModel>> {
  final PurchaseRepositoryImpl _r;
  final Ref _ref;
  PurchaseNotifier(this._r, this._ref) : super([]) { load(); }
  Future<void> load() async { state = await _r.getPurchases(); }
  Future<void> add(PurchaseModel p) async {
    await _r.addPurchase(p);
    await load();
