...
final collectionsProvider = StateNotifierProvider<CollectionNotifier, List<CollectionModel>>(
    (ref) => CollectionNotifier(ref.watch(collectionRepoProvider), ref));

class CollectionNotifier extends StateNotifier<List<CollectionModel>> {
  final CollectionRepositoryImpl _r;
  final Ref _ref;
  CollectionNotifier(this._r, this._ref) : super([]) { load(); }
  Future<void> load() async { state = await _r.getCollections(); }
  Future<void> add(CollectionModel c) async {
    await _r.addCollection(c);
    await load();
